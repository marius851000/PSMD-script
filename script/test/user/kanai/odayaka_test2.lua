...
function EventTriggerPush(symbol)
  if symbol == "ATLAS_OBJ" then
    WINDOW:SysMsg(1219904562)
    WINDOW:CloseMessage()
    SYSTEM:EnterPokemonAtlas()
  elseif CHARA:IsExist(symbol) then
    if symbol == "PARTY1" then
      WINDOW:DrawFace(20, 88, SymAct(symbol), FACE_TYPE.NORMAL)
      WINDOW:Talk(SymAct(symbol), 1671108593)
      WINDOW:CloseMessage()
    else
